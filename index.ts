...
class Student {
  ...
  payFee(amount: number) {
    ...
    console.log(
      chalk.yellow(
        `$${amount} fee paid successfully. Now ${this.name}'s account balance is ${this.balance}`
      )
    );
  }
...
class StudentManager {
  ...
  addStudent(name: string) {
    ...
    console.log(
      chalk.yellow(`Student: ${name} added successfully, ID: ${newStudent.id}`)
    );
  }
  ...
  enrollStudentInCourse(studentId: number, course: string) {
  ...
  while (true) {
    ...
    switch (choice.choice) {
      ...
      case "Enroll student in course":
        ...
        studentManager.enrollStudentInCourse(
          courseInput.studentId,
          courseInput.course
        );
        break;
